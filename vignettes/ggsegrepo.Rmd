...
The object should be a valid ggseg-atlas (check with `is_ggseg_atlas()` and `as_ggseg_atlas()`).
Likewise, `cc200_3d.rda` should load `cc200_3d` and contain a valid ggseg3d-atlas.
...
- If you only included one atlas type (ggseg or ggseg3d), remove the unneeded atlas from `data/atlas.rda`.
  That file contains information for both atlas types and will fail if only one is present.
