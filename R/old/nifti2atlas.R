...
#' \itemize{
#'  \item{step 1 - }{`atlas_vol2surf` -  converts volume to surface}
#'  \item{step 2 - }{`atlas_vol2label` - reads labels from volume}
#'  \item{step 3 - }{`atlas_lab2ctab` - creates ctab from labels}
#'  \item{step 4 - }{`atlas_tcl` - runs tkSurfer tcl script}
#'  \item{step 5 - }{`atlas_isolate` - isolates coloured regions from images}
#' }
#'
#' @return ggseg-atlas
#' @export
nifti_2_atlas <- function(
  nifti,
  outdir,
  annot_lab,
  atlas_name,
  ...
  if (1 %in% steps) {
    if (verbose) cli::cli_progress_step("Runnin step 1\n")
    atlas_vol2surf(infile, outdir, projfrac, verbose)
  }

  # labels ----
  if (2 %in% steps) {
    if (verbose) cli::cli_progress_step("Runnin step 2\n")
    k <- atlas_vol2label(annot_lab, outdir, verbose)
  }

  # ctab ----
  if (3 %in% steps) {
    if (verbose) cli::cli_progress_step("Runnin step 3\n")
    atlas_lab2ctab(outdir, verbose)
  }

  # tcl ----
  if (4 %in% steps) {
    if (verbose) cli::cli_progress_step("Runnin step 4\n")
    atlas_tcl(annot_lab, outdir, verbose)
  }

  # isolate colour ----
  if (5 %in% steps) {
    if (verbose) cli::cli_progress_step("Runnin step 5\n")
