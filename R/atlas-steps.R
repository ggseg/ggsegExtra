...
snapshot_region <- function(
  ...
  if (!file.exists(file.path(output_dir, outfile))) {
...
extract_contours <- function(
  ...
  raster_objects <- lapply(regions, rast)
  names(raster_objects) <- file_path_sans_ext(basename(regions))
  # Extract single max value
  maks <- global(raster_objects[[1]], fun = "max", na.rm = TRUE)
  contourobjs <- lapply(
    #   cli::cli_progress_along(
    raster_objects,
    #    name = "{step} Extracting contours"
    #  ),
    get_contours,
    max_val = maks[1, 1],
...
smooth_contours <- function(dir, smoothness, step, ncores = 2) {
  cli::cli_progress_step("{step} Smoothing contours")
...
reduce_vertex <- function(dir, tolerance) {
...
prep_labels <- function(
  ...
  if (file.exists(labels_list) && skip_existing) {
    ...
    p <- progressor(steps = nrow(all_combs))

    k <- future_lapply(
      X = seq_along(all_combs$label),
      FUN = function(x) {
        p()
        mri_vol2label(
          label_id = all_combs$label[x],
          hemisphere = all_combs$hemi[x],
    ...
    not_empty <- sapply(k, function(x) length(attr(x, "status")))
    all_combs <- all_combs[!as.logical(not_empty), ]
    all_combs$label <- sprintf("%04d", all_combs$label)

    labs <- list.files(
      output_dir,
      pattern = paste0(all_combs$label, collapse = "|"),
      full.names = TRUE
    )
  }
