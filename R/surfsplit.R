...
surfsplit <- function(
  ...
  if (is.null(dpx)) {
    ...
    if (!is.integer(dpx$V1)) {
      cli::cli_abort(
        'The DPV or DPF file must contain only integers\n',
        call. = FALSE
      )
    }
    ...
    if (facewise) {
      ...
      for (f in 1:nrow(surf$faces)) {
        ...
        if (length(unique(Cidx)) == 1) {
          # If all vertices share same label
          ...
          # If 2 or 3 vertices are in different labels
          ...
          if (is.null(getmode(Cidx)))
            print(paste0(f, ": mode is ", getmode(Cidx)))
  ...
  plys <- lapply(
    gsub("\\.dpv", "\\.ply", gsub("ascii", "ply", fname)),
    get_mesh,
    ShowSpecimen = FALSE
  )
