...
aparc_2_mesh <- function(
  ...
  dpv_files <- file.path(dirs[1], paste(hemisphere, annot, "dpv", sep = "."))

  if (!file.exists(dpv_files)) {
    dpx <- annot2dpv(
      annot_file,
      dpv_files,
      verbose = verbose
  ...
  if (surface == "LCBC" & subject == "fsaverage5") {
    surf_file <- system.file(
      "surfaces",
      paste0(hemisphere, ".LCBC"),
      package = "ggsegExtra"
    )
  } else {
    surf_file <- file.path(
  ...
  if (!file.exists(surf_file)) {
    cli::cli_abort(c(
      "cannot find specified surface file for this subject.",
  ...
  ascii_files <- file.path(
    dirs[3],
    paste(hemisphere, surface, "dpv", sep = ".")
  )
  if (!file.exists(ascii_files)) {
    surf <- surf2asc(
      surf_file,
      ascii_files,
      verbose = verbose
  ...
  ply_files <- file.path(dirs[4], paste(hemisphere, surface, "ply", sep = "."))
  if (!file.exists(ply_files)) {
    ply <- asc2ply(
      ascii_files,
      ply_files
    )
  ...
  # split into labels
  label_files <- file.path(
    dirs[1],
    paste(hemisphere, annot, "dpv", sep = ".")
  )
  ...
  if (length(plys) != nrow(colortable)) {
    ...
    lab_not_present <- sapply(
      colortable$code,
      function(x) any(x %in% ant$label)
    )
...
#'
#' @param annot annotation file, with name without the hemisphere information or the .annot extension, and be in annot_dir
#' @template subject
#' @template hemisphere
...
make_aparc_2_3datlas <- function(
  ...
  dt2 <- expand.grid(
    list(hemi = hemisphere, surf = surface),
  ...
  if (length(annot_file) == 0) {
    cli::cli_abort("Cannot find {.var {annot}}' in {.file {annot_dir}}")
  }

  p <- progressor(steps = nrow(dt2), label = "Extracting surface data")
  dt <- furrr::future_pmap(
    dt2,
    function(hemi, surf) {
      result <- aparc_2_mesh(
        hemisphere = hemi,
  ...
  if (any(idx)) {
    cli::cli_abort(c(
      "An error occurred when creating atlas.",
      dt[[idx]]
    ))
  }
...
aseg_2_mesh <- function(
  ...
  if (!check_fs()) {
    cli::cli_abort("Freesurfer not installed.")
  }

  if (!any(steps %in% 1:5)) {
    cli::cli_abort("{.code steps} must be numbers between 1 and 5.")
  }
  ...
  j <- sapply(dirs[!k], mkdir)

  if (any(!k) & length(j) == 0) {
    cli::cli_abort(c(
      "Unable to create output directories.",
      "Check if output directory parent is writeable.",
  ...
  if (2 %in% steps) {
    # tesselate
    mri_tessellate(
      file.path(dirs[3], paste0(lab_string, "_filled.mgz")),
      label,
  ...
  if (3 %in% steps) {
    ...
    mri_smooth(
      file.path(dirs[3], paste0(lab_string, "_notsmooth")),
      file.path(dirs[3], paste0(lab_string, "_smooth")),
      verbose = verbose
    )
  } #end 3
  ...
  if (verbose) {
    cat("Reading in mesh information\n")
  }
  ply <- get_mesh(
    file.path(dirs[2], paste0(lab_string, ".ply")),
    ShowSpecimen = FALSE
...
#' @template subject
#' @template subjects_dir
#' @param template template `volume.mgz` file path
#' @param color_lut Freesurfer colour look-up-table. Either
#' as a path or a data.frame that [is_ctab()]
...
make_volumetric_2_3datlas <- function(
  ...
  if (!freesurfer::have_fs()) {
    cli::cli_abort("FreeSurfer not installed. Cannot run pipeline")
  }
  ...
  labels <- unique(c(freesurfer::read_mgz(template)))
  ...
  if (!is.null(color_lut)) {
    ...
    colortable$colour = grDevices::rgb(
      colortable$R,
      colortable$G,
      colortable$B,
      maxColorValue = 255
  ...
  p <- progressor(steps = length(labels), label = "Creating meshes")
  plys <- furrr::future_map(
    labels,
    function(label) {
      result <- aseg_2_mesh(
        label = label,
  ...
  if (cleanup) {
    if (verbose) {
      cat(paste0("Cleaning up files in:", output_dir, "\n"))
    }
