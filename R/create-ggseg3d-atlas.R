...
make_aparc_2_3datlas <- function(
  ...
  # TODO: add  check if annot and surf match here. Fail in parallel
  # process is meaningless when debugging
  p <- progressr::progressor(
    along = dt2$hemi,
    message = "Extracting surface data"
  )
...
aseg_2_mesh <- function(
  ...
  if (!check_fs()) cli::cli_abort("Freesurfer not installed.")
  ...
  if (3 %in% steps) {
    ...
    k <- file.rename(
      file.path(dirs[3], paste0("rh.", label)),
