...
adjust_coords_sf <- function(atlas_df) {
  atlas_grouped <- group_by(atlas_df, hemi, side)
  keys <- group_keys(atlas_grouped)
  splits <- group_split(atlas_grouped)

  find_group <- function(h_vals, s) {
    idx <- which(keys$hemi %in% h_vals & keys$side == s)
    if (length(idx) == 1) splits[[idx]] else NULL
  }

  atlas <- list(
    find_group(c("left", "lh"), "lateral"),
    find_group(c("left", "lh"), "medial"),
    find_group(c("right", "rh"), "medial"),
  ...
  atlas <- lapply(seq_along(atlas), function(x) {
    ...
    tmp <- resize_coords_sf(atlas[[x]], sz[x])
    correct_coords_sf(tmp, offsets[x])
  })

  atlas <- atlas[!sapply(atlas, is.null)]
...
restack <- function(df) {
  ...
  max_extent <- max(sapply(rr, function(r) max(abs(r[, 2]))))
  widths <- sapply(rr, function(r) diff(r[, 1]))
  half_widths <- sapply(rr, function(r) max(abs(r[, 1])))
  gap <- max(widths) * 0.15
