...
#' usually a file made by for instance [surf2asc()],
#' [curv2asc()], [curvnf2asc()]
#'
...
is_facewise <- function(dpx, surface, verbose = TRUE) {
  ...
  if (nX == nV) {
    if (verbose) cli::cli_alert_info('Working with vertexwise data.\n')
    return(FALSE)
  } else if (nX == nF) {
    if (verbose) cli::cli_alert_info('Working with facewise data.\n')
    return(TRUE)
  } else {
