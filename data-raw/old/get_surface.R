...
rgb2hex = function(r = 0, g = 0, b = 0) {
  ...
  check = function(x, ix = NULL) {
    ...
    if (!is.numeric(x)) {
      cli::cli_abort(sprintf("'%s%s' must be numeric", nm, ix), call. = FALSE)
    }
    if (length(x) != 1) {
      cli::cli_abort(sprintf("'%s%s' must be scalar", nm, ix), call. = FALSE)
    }
    if (!is.finite(x)) {
      cli::cli_abort(sprintf("'%s%s' must be finite", nm, ix), call. = FALSE)
    }
    if (x < 0 | x > 255) {
      cli::cli_abort(
        sprintf("'%s%s' must be in the range [0,255]", nm, ix),
        call. = FALSE
      )
    }
  }
